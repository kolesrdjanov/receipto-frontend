...
import { ArrowLeft, CircleAlert, Check, Eye, EyeOff, Loader2, type LucideIcon } from 'lucide-react'
...
export const GlassField = React.forwardRef<HTMLInputElement, GlassFieldProps>(
  ({ label, icon: Icon, error, invalid, trailing, id, name, className, containerClassName, ...props }, ref) => {
    ...
    return (
      <div className={cn('min-w-0 flex-1', containerClassName)}>
        ...
        >
          {Icon && <Icon className="size-[17px] shrink-0 text-muted-foreground/70" />}
          <input
            id={inputId}
            name={name}
            ref={ref}
...
export const PasswordField = React.forwardRef<HTMLInputElement, Omit<GlassFieldProps, 'trailing' | 'type'>>(
  (props, ref) => {
...
const STRENGTH = [
  ...
  { key: 'auth.passwordStrength.fair', bar: 'bg-amber-500', text: 'text-amber-600 dark:text-amber-400' },
  { key: 'auth.passwordStrength.good', bar: 'bg-blue-500', text: 'text-blue-600 dark:text-blue-400' },
  { key: 'auth.passwordStrength.strong', bar: 'bg-emerald-500', text: 'text-emerald-600 dark:text-emerald-400' },
] as const
...
export function PasswordStrengthMeter({ value }: { value: string }) {
  ...
  return (
    <div className="mt-2.5 flex items-center gap-2">
      ...
      <span
        className={cn(
          'min-w-[42px] text-right text-[11.5px] font-semibold',
          meta ? meta.text : 'text-muted-foreground/70',
        )}
      >
        {meta ? t(meta.key) : '—'}
      </span>
...
export function AuthBadge({ icon: Icon, kind = 'primary' }: { icon: LucideIcon; kind?: BadgeKind }) {
  return (
  ...
  return (
    <div className="mb-[22px] text-center">
      {logo ? (
        ...
        <AuthBadge icon={badge} kind={badgeKind} />
      ) : null}
...
/* ------------------------------------------------------------------ */
type AuthCheckboxProps = Omit<React.InputHTMLAttributes<HTMLInputElement>, 'type'>

export function AuthCheckbox({ className, checked, ...props }: AuthCheckboxProps) {
  return (
