  ...
  return (
    <AppLayout>
      ...
      <div className="flex flex-col gap-4 mb-6 sm:flex-row sm:items-center sm:justify-between sm:mb-8">
        <div>
          <h2 className="text-2xl font-bold tracking-tight mb-1 sm:text-3xl sm:mb-2">{t('warranties.title')}</h2>
          <p className="text-muted-foreground">{t('warranties.subtitle')}</p>
        </div>
        <div className="flex flex-wrap gap-4 sm:gap-2">
          <Button className={'w-full sm:w-auto'} variant="outline" onClick={handleExport} disabled={exportWarranties.isPending}>
            <Download className="h-4 w-4" />
            {t('warranties.export.button')}
          </Button>
          <Button className={'w-full sm:w-auto'} variant="outline" onClick={handleImportClick} disabled={importWarranties.isPending}>
            <Upload className="h-4 w-4" />
            {t('warranties.import.button')}
          </Button>
          <Button className={'w-full sm:w-auto'} onClick={handleAddWarranty}>
